from __future__ import annotations

import argparse
from pathlib import Path

from public_release import (
    build_public_bundle,
    regenerate_known_withheld_manifest,
    scan_static_output,
)


ROOT = Path(__file__).resolve().parents[1]
WITHHELD_SENTINEL = "WITHHELD_SENTINEL_DO_NOT_SHIP"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate, stage and scan the public-only static data bundle."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "public-release",
        help="Generated output directory (default: build/public-release).",
    )
    parser.add_argument(
        "--known-withheld-catalog",
        type=Path,
        required=True,
        help="Candidate values keyed by fact ID.",
    )
    parser.add_argument(
        "--known-withheld-manifest",
        type=Path,
        required=True,
        help="Gitignored scan manifest regenerated by this command.",
    )
    parser.add_argument(
        "--export-output",
        type=Path,
        help="Existing Astro dist directory to scan after the pre-application bundle.",
    )
    args = parser.parse_args()

    projection_path = ROOT / "public" / "data" / "projects.json"
    regenerate_known_withheld_manifest(
        projection_path=projection_path,
        candidate_catalog_path=args.known_withheld_catalog,
        manifest_path=args.known_withheld_manifest,
    )

    build_public_bundle(
        projection_path=projection_path,
        schema_path=ROOT / "public" / "data" / "public-projection.schema.json",
        boundary_path=ROOT / "public" / "data" / "map" / "berlin-boundary.geojson",
        boundary_provenance_path=(
            ROOT / "public" / "data" / "map" / "berlin-boundary.provenance.json"
        ),
        review_decisions_path=(
            ROOT / "public" / "data" / "accepted-review-decisions.json"
        ),
        name_allowlist_path=ROOT / "public" / "data" / "name-allowlist.json",
        output_dir=args.output,
    )
    scanned = scan_static_output(
        args.output,
        sentinels=(WITHHELD_SENTINEL,),
        known_withheld_manifest=args.known_withheld_manifest,
        projection_path=projection_path,
    )
    print(f"public bundle valid; scanned {len(scanned)} generated files")
    if args.export_output is not None:
        exported = scan_static_output(
            args.export_output,
            sentinels=(WITHHELD_SENTINEL,),
            known_withheld_manifest=args.known_withheld_manifest,
            projection_path=projection_path,
        )
        print(f"Astro export valid; scanned {len(exported)} generated files")
        for path in exported:
            print(path)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
